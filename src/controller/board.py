...
class Board:
    ...
    def logical_to_pixel(self, pos):
        col, row = pos
        x = config.MARGIN_X + row * config.CELL_SIZE
        y = config.MARGIN_Y + col * config.CELL_SIZE
        return (x, y)
    ...
    def pixel_to_logical(self, pos):
        x, y = pos
        row = round((x - config.MARGIN_X) / config.CELL_SIZE)
        col = round((y - config.MARGIN_Y) / config.CELL_SIZE)
    ...
    def draw(self):
        ...
        for row in range(config.BOARD_ROWS):
            for col in range(config.BOARD_ROWS):
                x = config.MARGIN_X + col * config.CELL_SIZE
                y = config.MARGIN_Y + row * config.CELL_SIZE

                # Horizontal lines
                if col < config.BOARD_ROWS - 1:
                    pygame.draw.line(
                        self.surface, config.WHITE,
        ...
        for (lx, ly) in self.points:
            ...
            pygame.draw.circle(self.surface, config.WHITE, (px, py), config.POINT_RADIUS_EMPTY+2, 1)
            pygame.draw.circle(self.surface, config.WHITE, (px, py), config.POINT_RADIUS_EMPTY)

        # PIECES
        for piece in self.player1_pieces:
            px, py = self.logical_to_pixel(piece.position)
            pygame.draw.circle(self.surface, config.PLAYER1, (px, py), config.POINT_RADIUS+3, 1)
            pygame.draw.circle(self.surface, config.PLAYER1, (px, py), config.POINT_RADIUS)

        for piece in self.player2_pieces:
            px, py = self.logical_to_pixel(piece.position)
            pygame.draw.circle(self.surface, config.PLAYER2, (px, py), config.POINT_RADIUS+3, 1)
            pygame.draw.circle(self.surface, config.PLAYER2, (px, py), config.POINT_RADIUS)
